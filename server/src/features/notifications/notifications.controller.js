...
async function listNotifications(req, res) {
  ...
  const data = await svc.listNotifications(req.user?.userId || req.user?.id, { page, pageSize, isRead });
  return successResponse(res, 'Notifications retrieved', data);
}

async function markNotificationRead(req, res) {
  const result = await svc.markAsRead(req.params.notificationId, req.user?.userId || req.user?.id);
...
async function markAllNotificationsRead(req, res) {
  const result = await svc.markAllAsRead(req.user?.userId || req.user?.id);
