...
export const api = {
  ...
  async getProducts() {
    try {
      ...
      return {
        success: false,
        error: `Backend HTTP Error ${res.status}: ${res.statusText || 'Failed to fetch products'}`,
        status: res.status
      };
    } catch (e) {
      console.info('[API Offline Mode] Backend unreachable, using mock products:', e.message);
  ...
  async getCustomers() {
    try {
      ...
      return {
        success: false,
        error: `Backend HTTP Error ${res.status}: ${res.statusText || 'Failed to fetch customers'}`,
        status: res.status
      };
    } catch (e) {
      console.info('[API Offline Mode] Backend unreachable, using mock customers:', e.message);
  ...
  async getWarehouses() {
    try {
      ...
      return {
        success: false,
        error: `Backend HTTP Error ${res.status}: ${res.statusText || 'Failed to fetch warehouses'}`,
        status: res.status
      };
    } catch (e) {
